...
function returnsImmutableObject(name, config) {
    assert.ok(Object(config) === config && !Array.isArray(config), `Static method '${name}' did not return an object`);
    ...
    describe("SCIMMY.Config", () => {
        ...
        describe(".set", () => {
            ...
            it("should do nothing without arguments", () => {
                ...
                assert.deepStrictEqual(SCIMMY.Config.set(), config, "Static method 'set' unexpectedly modified config");
            });
