...
export let BulkOpSuite = (SCIMMY) => {
    ...
    const fixtures = fs.readFile(path.join(basepath, "./bulkop.json"), "utf8").then((f) => JSON.parse(f));
    const params = {id: "urn:ietf:params:scim:api:messages:2.0:BulkRequest"};
    const template = {schemas: [params.id], Operations: [{}, {}]};
    
    /** 
     * BulkOp Test Resource Class
     * Because BulkOp needs a set of implemented resources to test against 
     */
    ...
    describe("SCIMMY.Messages.BulkOp", () => {
        ...
        describe("#apply()", () => {
            ...
            it("should expect 'method' attribute to have a value for each operation", async () => {
                let actual = (await (new SCIMMY.Messages.BulkOp({...template, Operations: [{}, {path: "/Test"}, {method: ""}]})).apply())?.Operations,
            ...
            it("should expect 'method' attribute to be a string for each operation", async () => {
                ...
                for (let [label, value] of fixtures) {
                    let actual = (await (new SCIMMY.Messages.BulkOp({...template, Operations: [{method: value}]})).apply())?.Operations,
            ...
            it("should expect 'method' attribute to be one of POST, PUT, PATCH, or DELETE for each operation", async () => {
                let actual = (await (new SCIMMY.Messages.BulkOp({...template, Operations: [{method: "a string"}]})).apply())?.Operations,
            ...
            it("should expect 'path' attribute to have a value for each operation", async () => {
                let actual = (await (new SCIMMY.Messages.BulkOp({...template, Operations: [{method: "POST"}, {method: "POST", path: ""}]})).apply())?.Operations,
            ...
            it("should expect 'path' attribute to be a string for each operation", async () => {
                ...
                for (let [label, value] of fixtures) {
                    let actual = (await (new SCIMMY.Messages.BulkOp({...template, Operations: [{method: "POST", path: value}]})).apply())?.Operations,
            ...
            it("should expect 'path' attribute to refer to a valid resource type endpoint", async () => {
                let actual = (await (new SCIMMY.Messages.BulkOp({...template, Operations: [{method: "POST", path: "/Test"}]})).apply())?.Operations,
                    expected = [{status: "400", method: "POST", response: {
            ...
            it("should expect 'path' attribute to NOT specify a resource ID if 'method' is POST", async () => {
                let actual = (await (new SCIMMY.Messages.BulkOp({...template, Operations: [{method: "POST", path: "/Test/1", bulkId: "asdf"}]})).apply([Test]))?.Operations,
            ...
            it("should expect 'path' attribute to specify a resource ID if 'method' is not POST", async () => {
                let actual = (await (new SCIMMY.Messages.BulkOp({...template, Operations: [{method: "PUT", path: "/Test"}, {method: "DELETE", path: "/Test"}]})).apply([Test]))?.Operations,
            ...
            it("should expect 'bulkId' attribute to have a value for each 'POST' operation", async () => {
                let actual = (await (new SCIMMY.Messages.BulkOp({...template, Operations: [{method: "POST", path: "/Test"}, {method: "POST", path: "/Test", bulkId: ""}]})).apply([Test]))?.Operations,
            ...
            it("should expect 'bulkId' attribute to be a string for each 'POST' operation", async () => {
                ...
                for (let [label, value] of fixtures) {
                    let actual = (await (new SCIMMY.Messages.BulkOp({...template, Operations: [{method: "POST", path: "/Test", bulkId: value}]})).apply([Test]))?.Operations,
                        expected = [{status: "400", method: "POST", response: {
                            ...new SCIMMY.Messages.Error(new SCIMMY.Types.Error(
            ...
            it("should expect 'data' attribute to have a value when 'method' is not DELETE", async () => {
                let actual = (await (new SCIMMY.Messages.BulkOp({...template, Operations: [{method: "POST", path: "/Test", bulkId: "asdf"}, {method: "PUT", path: "/Test/1"}, {method: "PATCH", path: "/Test/1"}]})).apply([Test]))?.Operations,
            ...
            it("should expect 'data' attribute to be a single complex value when 'method' is not DELETE", async () => {
                ...
                for (let op of suite) {
                    for (let [label, value] of fixtures) {
                        let actual = (await (new SCIMMY.Messages.BulkOp({...template, Operations: [{...op, data: value}]})).apply([Test]))?.Operations,
            ...
            it("should stop processing operations when failOnErrors limit is reached", async () => {
                ...
                for (let fixture of suite) {
                    let result = await (new SCIMMY.Messages.BulkOp({...template, ...fixture.source})).apply([Test.reset()]);
